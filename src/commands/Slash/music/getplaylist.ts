...
module.exports = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const songs = await getPlaylist(interaction.client.dbPool, userinfo.id);
    ...
    if(songsData.length == 0){
        await interaction.editReply({embeds: [embedError("Unable to Fetch Data of Any Song")]});
        return;
    }

    // Creating Embed and Adding Song Data to it
    const embed = new EmbedBuilder().setTitle(`${userinfo.username}'s Playlist`);
    for(let idx =0; idx < songsData.length; idx++){
        embed.addFields({name: `Song Duration: **${secondsToString(songsData[idx].length)}**`, value: `[${songsData[idx].title.toUpperCase()}](${songsData[idx].url})`});
    }
