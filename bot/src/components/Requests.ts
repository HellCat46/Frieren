...
export async function createTopic(topicName : string, pageurl ?: string) : Promise<{id : number, msg : string}> {
    if(!pageurl) pageurl = "";
    try {
...
export async function getPageLink(topicId : number, pageno : number) : Promise<string> {
    try{
        const res = await fetch(
...
> {
    ...
    let collection : Collection<number, { name: string; page_count: number }> = new Collection();
    if(res.status == 200){
        const json : {list : {id : number, name : string, page_count : number}[]} = await res.json();
        json.list.forEach(topic => {
            collection.set(topic.id, {name : topic.name, page_count : topic.page_count});
